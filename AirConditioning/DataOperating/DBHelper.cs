...
{
    public static string connString = ConfigurationManager.AppSettings["connectionString"].Trim();
    public static string connStringMain = ConfigurationManager.AppSettings["connectionStringMain"].Trim();
    #region 私有构造函数和方法
    private DBHelper() { }
    /// <summary>
    ...
    public static int ExecuteNonQuery(string connectionString, string spName, params object[] parameterValues)
    {
        ...
        if ((parameterValues != null) && (parameterValues.Length > 0))
        {
            ...
            MySqlParameter[] commandParameters = DBHelperParameterCache.GetSpParameterSet(connectionString, spName);
            // 给存储过程参数赋值
    ...
    public static int ExecuteNonQuery(MySqlConnection connection, string spName, params object[] parameterValues)
    {
        ...
        {
            // 从缓存中加载存储过程参数
            MySqlParameter[] commandParameters = DBHelperParameterCache.GetSpParameterSet(connection, spName);
            // 给存储过程分配参数值
            AssignParameterValues(commandParameters, parameterValues);
    ...
    public static int ExecuteNonQuery(MySqlTransaction transaction, string spName, params object[] parameterValues)
    {
        ...
        if ((parameterValues != null) && (parameterValues.Length > 0))
        {
            ...
            MySqlParameter[] commandParameters = DBHelperParameterCache.GetSpParameterSet(transaction.Connection, spName);
            // 给存储过程参数赋值
            AssignParameterValues(commandParameters, parameterValues);
            // 调用重载方法
    ...
    /// <param>标识数据库连接对象是由调用者提供还是由DBHelper提供</param>
